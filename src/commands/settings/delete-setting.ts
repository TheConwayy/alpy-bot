...
export class DeleteSettingCommand extends Command {
	...
	public override async messageRun(message: Message, args: Args) {
		...
		if (!setting) {
			const errorContainer = new MessageContainer().setHeading('Error', Emojis.invalid).setBody('Please provide a setting to delete.');
		...
		const messageContainer = new MessageContainer()
			...
			.setBody(
				success ? `Successfully deleted setting \`${setting}\`` : `Failed to delete setting \`${setting}\`.\n**\`Error:\`** ${result.error}`
			);
