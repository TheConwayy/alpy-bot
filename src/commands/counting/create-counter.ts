...
export class CreateCounterCommand extends Command {
	...
	public override async messageRun(message: Message) {
		const answers = await runSurvey(message, questions, 'Please answer the following questions:', 'All questions have been answered!');

		if (!answers.counter_name || !answers.counter_description) {
			return await message.reply('Please provide a name and description for the counter.');
		}

		const formattedAnswer = answers.counter_name.split(' ').join('_').toLowerCase();

		const { success, error } = await createCounter(message.author.id, formattedAnswer, answers.counter_description);
