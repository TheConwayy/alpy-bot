...
export async function checkRoutingNumber(routingNumber: string): Promise<RoutingNumberResult> {
	...
	// Check cache
	const { data: cachedData } = await supabase.from('routing_numbers').select('*').eq('routing_number', routingNumber).single();
	...
	try {
		...
		try {
			const { error: upsertError } = await supabase.from('routing_numbers').upsert({
